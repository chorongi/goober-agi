...
- **Construction**: Evaluates predictive social cognition. The model is provided with a **density and duration guaranteed context** (20 messages and 30s video). To ensure a balanced level of difficulty, the model predicts a **representative block of 5-10 chat messages** that would likely appear next, rather than a line-by-line prediction.
- **Noise Reduction**: Uses a multimodal 0.5 FPS context (224x224) to focus on key visual shifts without overwhelming the model with redundant frames.
- **Statistical Stability**: The benchmark performs **3 consecutive trials** per stream and averages the results to smooth out momentary lulls.

### Task 2: Cognitive Reconstruction (Visual Inference)
- **Construction**: A 10s video window is withheld while chat remains visible. To ensure stability, the benchmark uses a **density and duration guaranteed context** and performs **3 consecutive trials** per stream, averaging the scores.
- **Unambiguous Ground Truth**: Evaluation compares the prediction against a **Ground Truth Reconstruction** generated by the LLM from the actual withheld frames. 
