...
def future_chat_prediction(llm) -> float:
    ...
    for video_url in all_videos:
        ...
        try:
            ...
            # so the timestamps represent the immediate 0~10 seconds 'future'
            ...
            # Hand the text prompt and the VideoContent object to the LLM
            ...
            if assessment is not None:
                ...
                for result in assessment.results:
                    # Log assertion failure if any criterion fails
...
def past_frame_generation(llm) -> float:
    ...
    for video_url in all_videos:
        ...
        try:
            ...
            # 1. Provide 30s of initial context (Video + Chat)
            ...
            # 2. WITHELD SEGMENT: Capture 10s of chat (input) and 10s of video (ground truth)
            ...
            # 3. Prompt AGI to "Reconstruct" the visuals via detailed descriptions
            prompt = textwrap.dedent(f\"\"\"
            ...
            # Hand the text prompt and the context video to the LLM
...
def stream_switch_adaptation(llm) -> float:
    ...
    # Test switches between consecutive streams in the list
    for i in range(len(all_videos)):
        ...
        try:
            # 1. Establish baseline context on Stream A (Warm up)
            ...
            # Hand the text prompt and the 5s glance video to the LLM
            zero_shot_prediction = llm.prompt(message=prompt, video=glance_video)

            # 6. Define Adaptation Criteria
            criteria = [
...
def make_cell(source_code):
    ...
    # Add an actual newline character '\n' to every string in the list except the very last one 
    # to perfectly match Jupyter Notebook list-of-strings formatting
...
def create_notebook(filename, task_code):
    cells = [
        make_cell("!pip install -q yt-dlp pytchat opencv-python-headless numpy Pillow"),
        make_cell(FETCHER_CODE),
        make_cell(VIDEO_SOURCES_CODE),
        make_cell(task_code)
    ]
