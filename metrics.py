...
try:
    ...
    print("Warning: ML Evaluation libraries (rouge_score, bert_score, lpips, clip) not fully installed. Falling back to mock metrics for structural testing.")
...
class Evaluator:
    ...
    def evaluate_task1(self, pred_chat: List[str], true_chat: List[str]) -> float:
        ...
        if not METRICS_AVAILABLE:
            return np.random.uniform(50, 90) # Mock score
            
        # 1. ROUGE-L
        rouge_scores = self.rouge_scorer.score(true_text, pred_text)
    ...
    def evaluate_task2(self, pred_frames: List[np.ndarray], true_frames: List[np.ndarray], chat_context: List[str]) -> float:
        """Returns a normalized score (0-100) using LPIPS and CLIP alignment."""
        if not pred_frames or not true_frames:
            return 0.0
            
        if not METRICS_AVAILABLE:
             return np.random.uniform(40, 85) # Mock score
        ...
        text_input = clip.tokenize([chat_text[:77]]).to(self.device) # CLIP max context is 77 tokens

        with torch.no_grad():
            for p_img, t_img in zip(pred_frames, true_frames):
                # Convert to PIL then to torch tensors
                p_pil = Image.fromarray(p_img)
                t_pil = Image.fromarray(t_img)
                
                # 1. LPIPS Calculation
                # LPIPS expects tensors in [-1, 1]
                p_tensor = torch.tensor(p_img).permute(2, 0, 1).unsqueeze(0).float() / 127.5 - 1
                t_tensor = torch.tensor(t_img).permute(2, 0, 1).unsqueeze(0).float() / 127.5 - 1
                
                loss = self.lpips_loss_fn_vgg(p_tensor.to(self.device), t_tensor.to(self.device))
                lpips_scores.append(loss.item())
                
                # 2. CLIP Alignment Calculation
                image_input = self.clip_preprocess(p_pil).unsqueeze(0).to(self.device)
        ...
        lpips_score_normalized = max(0, (1.0 - avg_lpips)) * 100
        
        # Normalize CLIP (-1 to 1) -> Convert to 0-100
        avg_clip = np.mean(clip_scores)
        clip_score_normalized = ((avg_clip + 1) / 2) * 100
